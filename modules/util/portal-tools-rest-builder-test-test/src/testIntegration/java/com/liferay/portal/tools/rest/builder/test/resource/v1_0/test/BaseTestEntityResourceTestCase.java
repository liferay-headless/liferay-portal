...
public abstract class BaseTestEntityResourceTestCase {
	...
	@Test
	public void testGetTestEntityMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		TestEntity postTestEntity = testGetTestEntity_addTestEntity();

		String endpoint = StringBundler.concat(
			"/test/v1.0/test-entities/{testEntityId}".replace(
				"{testEntityId}", String.valueOf(postTestEntity.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/test-entities/{testEntityId}".replace(
								"{testEntityId}",
								String.valueOf(postTestEntity.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					TestEntitySerDes.toDTO(apiResponse),
					TestEntitySerDes.toDTO(vulcanResponse)));
		}
	}
