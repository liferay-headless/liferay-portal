...
public abstract class BaseNotificationQueueEntryResourceTestCase {
	...
	@Test
	public void testGetNotificationQueueEntriesPageWithSortCollection()
		throws Exception {

		JSONObject xSortableJSONObject = HTTPTestUtil.invokeToJSONObject(
			null, "notification/v1.0/openapi.json", Http.Method.GET
		).getJSONObject(
			"components"
		).getJSONObject(
			"schemas"
		...
		for (EntityField entityField :
			...
			try {
				Method method = notificationQueueEntry1.getClass(
				).getMethod(
					"get" + StringUtil.upperCaseFirstLetter(entityFieldName)
				);
				...
				if (returnType.equals(Long.class)) {
					BeanTestUtil.setProperty(
						notificationQueueEntry1, entityFieldName, 0L);
					BeanTestUtil.setProperty(
						notificationQueueEntry2, entityFieldName, 1L);
				}
				else if (returnType.equals(Integer.class)) {
					BeanTestUtil.setProperty(
						notificationQueueEntry1, entityFieldName, 0);
					BeanTestUtil.setProperty(
						notificationQueueEntry2, entityFieldName, 1);
				}
...
// LIFERAY-REST-BUILDER-HASH:2020572866