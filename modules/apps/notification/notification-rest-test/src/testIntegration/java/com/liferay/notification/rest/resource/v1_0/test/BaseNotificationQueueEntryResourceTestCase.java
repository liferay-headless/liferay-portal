...
public abstract class BaseNotificationQueueEntryResourceTestCase {
	...
	@Test
	public void testGetNotificationQueueEntryMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {
		...
		String endpoint = StringBundler.concat(
			"/notification/v1.0/notification-queue-entries/{notificationQueueEntryId}".
				replace(
					"{notificationQueueEntryId}",
					String.valueOf(postNotificationQueueEntry.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/notification-queue-entries/{notificationQueueEntryId}".
								replace(
									"{notificationQueueEntryId}",
									String.valueOf(
										postNotificationQueueEntry.getId()));
			...
			Assert.assertTrue(
				equals(
					NotificationQueueEntrySerDes.toDTO(apiResponse),
					NotificationQueueEntrySerDes.toDTO(vulcanResponse)));
		}
	}
