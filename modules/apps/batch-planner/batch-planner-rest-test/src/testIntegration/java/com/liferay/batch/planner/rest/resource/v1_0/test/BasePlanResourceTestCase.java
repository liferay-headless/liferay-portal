...
public abstract class BasePlanResourceTestCase {
	...
	@Test
	public void testGetPlanMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Plan postPlan = testGetPlan_addPlan();

		String endpoint = StringBundler.concat(
			"/batch-planner/v1.0/plans/{planId}".replace(
				"{planId}", String.valueOf(postPlan.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/plans/{planId}".replace(
								"{planId}", String.valueOf(postPlan.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					PlanSerDes.toDTO(apiResponse),
					PlanSerDes.toDTO(vulcanResponse)));
		}
	}
