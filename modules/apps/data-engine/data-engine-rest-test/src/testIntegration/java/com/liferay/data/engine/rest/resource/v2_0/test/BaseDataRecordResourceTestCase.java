...
public abstract class BaseDataRecordResourceTestCase {
	...
	@Test
	public void testGetDataRecordMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		DataRecord postDataRecord = testGetDataRecord_addDataRecord();

		String endpoint = StringBundler.concat(
			"/data-engine/v2.0/data-records/{dataRecordId}".replace(
				"{dataRecordId}", String.valueOf(postDataRecord.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v2.0/data-records/{dataRecordId}".replace(
								"{dataRecordId}",
								String.valueOf(postDataRecord.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					DataRecordSerDes.toDTO(apiResponse),
					DataRecordSerDes.toDTO(vulcanResponse)));
		}
	}
