...
public abstract class BaseAuditEventResourceTestCase {
	...
	@Test
	public void testGetAuditEventsPageWithSortCollection() throws Exception {
		JSONObject xSortableJSONObject = HTTPTestUtil.invokeToJSONObject(
			null, "audit/v1.0/openapi.json", Http.Method.GET
		).getJSONObject(
			"components"
		).getJSONObject(
			"schemas"
		...
		for (EntityField entityField :
			...
			try {
				Method method = auditEvent1.getClass(
				).getMethod(
					"get" + StringUtil.upperCaseFirstLetter(entityFieldName)
				);
				...
				if (returnType.equals(Long.class)) {
					BeanTestUtil.setProperty(auditEvent1, entityFieldName, 0L);
					BeanTestUtil.setProperty(auditEvent2, entityFieldName, 1L);
				}
				else if (returnType.equals(Integer.class)) {
					BeanTestUtil.setProperty(auditEvent1, entityFieldName, 0);
					BeanTestUtil.setProperty(auditEvent2, entityFieldName, 1);
				}
...
// LIFERAY-REST-BUILDER-HASH:2112829845