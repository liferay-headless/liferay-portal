...
public abstract class BaseCTEntryResourceTestCase {
	...
	@Test
	public void testGetCTEntriesHistoryPageWithSortCollection()
		throws Exception {

		JSONObject xSortableJSONObject = HTTPTestUtil.invokeToJSONObject(
			null, "change-tracking-rest/v1.0/openapi.json", Http.Method.GET
		).getJSONObject(
			"components"
		).getJSONObject(
			"schemas"
		...
		for (EntityField entityField :
			...
			try {
				Method method = ctEntry1.getClass(
				).getMethod(
					"get" + StringUtil.upperCaseFirstLetter(entityFieldName)
				);
				...
				if (returnType.equals(Long.class)) {
					BeanTestUtil.setProperty(ctEntry1, entityFieldName, 0L);
					BeanTestUtil.setProperty(ctEntry2, entityFieldName, 1L);
				}
				else if (returnType.equals(Integer.class)) {
					BeanTestUtil.setProperty(ctEntry1, entityFieldName, 0);
					BeanTestUtil.setProperty(ctEntry2, entityFieldName, 1);
				}
	...
	@Test
	public void testGetCtCollectionCTEntriesPageWithSortCollection()
		throws Exception {
...
// LIFERAY-REST-BUILDER-HASH:-1005002677