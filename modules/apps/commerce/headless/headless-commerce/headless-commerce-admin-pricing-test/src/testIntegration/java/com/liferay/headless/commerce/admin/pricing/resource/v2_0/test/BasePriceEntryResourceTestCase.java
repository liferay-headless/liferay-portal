...
public abstract class BasePriceEntryResourceTestCase {
	...
	@Test
	public void testGetPriceEntryMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		PriceEntry postPriceEntry = testGetPriceEntry_addPriceEntry();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-pricing/v2.0/price-entries/{priceEntryId}".
				replace(
					"{priceEntryId}", String.valueOf(postPriceEntry.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v2.0/price-entries/{priceEntryId}".replace(
								"{priceEntryId}",
								String.valueOf(postPriceEntry.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					PriceEntrySerDes.toDTO(apiResponse),
					PriceEntrySerDes.toDTO(vulcanResponse)));
		}
	}
