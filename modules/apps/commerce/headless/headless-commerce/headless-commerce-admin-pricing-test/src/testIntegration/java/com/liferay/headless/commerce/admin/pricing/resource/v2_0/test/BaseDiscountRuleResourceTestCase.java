...
public abstract class BaseDiscountRuleResourceTestCase {
	...
	@Test
	public void testGetDiscountRuleMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		DiscountRule postDiscountRule = testGetDiscountRule_addDiscountRule();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-pricing/v2.0/discount-rules/{id}".replace(
				"{discountRuleId}", String.valueOf(postDiscountRule.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v2.0/discount-rules/{id}".replace(
								"{discountRuleId}",
								String.valueOf(postDiscountRule.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					DiscountRuleSerDes.toDTO(apiResponse),
					DiscountRuleSerDes.toDTO(vulcanResponse)));
		}
	}
