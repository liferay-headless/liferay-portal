...
public abstract class BaseTierPriceResourceTestCase {
	...
	@Test
	public void testGetTierPriceMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		TierPrice postTierPrice = testGetTierPrice_addTierPrice();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-pricing/v2.0/tier-prices/{id}".replace(
				"{tierPriceId}", String.valueOf(postTierPrice.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v2.0/tier-prices/{id}".replace(
								"{tierPriceId}",
								String.valueOf(postTierPrice.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					TierPriceSerDes.toDTO(apiResponse),
					TierPriceSerDes.toDTO(vulcanResponse)));
		}
	}
