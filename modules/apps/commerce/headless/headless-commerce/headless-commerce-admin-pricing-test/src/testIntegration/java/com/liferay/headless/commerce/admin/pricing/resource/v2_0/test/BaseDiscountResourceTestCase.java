...
public abstract class BaseDiscountResourceTestCase {
	...
	@Test
	public void testGetDiscountMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Discount postDiscount = testGetDiscount_addDiscount();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-pricing/v2.0/discounts/{id}".replace(
				"{discountId}", String.valueOf(postDiscount.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v2.0/discounts/{id}".replace(
								"{discountId}",
								String.valueOf(postDiscount.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					DiscountSerDes.toDTO(apiResponse),
					DiscountSerDes.toDTO(vulcanResponse)));
		}
	}
