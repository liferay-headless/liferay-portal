...
public abstract class BasePriceListResourceTestCase {
	...
	@Test
	public void testGetPriceListsPageWithSortCollection() throws Exception {
		JSONObject xSortableJSONObject = HTTPTestUtil.invokeToJSONObject(
			null, "headless-commerce-admin-pricing/v1.0/openapi.json",
			Http.Method.GET
		).getJSONObject(
			"components"
		).getJSONObject(
		...
		for (EntityField entityField :
			...
			try {
				Method method = priceList1.getClass(
				).getMethod(
					"get" + StringUtil.upperCaseFirstLetter(entityFieldName)
				);
				...
				if (returnType.equals(Long.class)) {
					BeanTestUtil.setProperty(priceList1, entityFieldName, 0L);
					BeanTestUtil.setProperty(priceList2, entityFieldName, 1L);
				}
				else if (returnType.equals(Integer.class)) {
					BeanTestUtil.setProperty(priceList1, entityFieldName, 0);
					BeanTestUtil.setProperty(priceList2, entityFieldName, 1);
				}
...
// LIFERAY-REST-BUILDER-HASH:1408329217