...
public abstract class BaseShipmentResourceTestCase {
	...
	@Test
	public void testGetShipmentMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Shipment postShipment = testGetShipment_addShipment();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-shipment/v1.0/shipments/{shipmentId}".
				replace("{shipmentId}", String.valueOf(postShipment.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/shipments/{shipmentId}".replace(
								"{shipmentId}",
								String.valueOf(postShipment.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					ShipmentSerDes.toDTO(apiResponse),
					ShipmentSerDes.toDTO(vulcanResponse)));
		}
	}
