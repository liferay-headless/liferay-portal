...
public abstract class BaseAccountAddressResourceTestCase {
	...
	@Test
	public void testGetAccountAddressMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {
		...
		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-account/v1.0/accountAddresses/{id}".
				replace(
					"{accountAddressId}",
					String.valueOf(postAccountAddress.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/accountAddresses/{id}".replace(
								"{accountAddressId}",
								String.valueOf(postAccountAddress.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					AccountAddressSerDes.toDTO(apiResponse),
					AccountAddressSerDes.toDTO(vulcanResponse)));
		}
	}
