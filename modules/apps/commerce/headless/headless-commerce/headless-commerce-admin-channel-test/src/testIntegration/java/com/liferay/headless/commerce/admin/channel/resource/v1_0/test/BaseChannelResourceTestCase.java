...
public abstract class BaseChannelResourceTestCase {
	...
	@Test
	public void testGetChannelMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Channel postChannel = testGetChannel_addChannel();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-channel/v1.0/channels/{channelId}".
				replace("{channelId}", String.valueOf(postChannel.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/channels/{channelId}".replace(
								"{channelId}",
								String.valueOf(postChannel.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					ChannelSerDes.toDTO(apiResponse),
					ChannelSerDes.toDTO(vulcanResponse)));
		}
	}
