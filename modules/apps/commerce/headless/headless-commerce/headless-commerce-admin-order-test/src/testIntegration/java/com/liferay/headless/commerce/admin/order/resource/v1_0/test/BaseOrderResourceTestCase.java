...
public abstract class BaseOrderResourceTestCase {
	...
	@Test
	public void testGetOrderMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Order postOrder = testGetOrder_addOrder();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-order/v1.0/orders/{id}".replace(
				"{orderId}", String.valueOf(postOrder.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
			...
			Assert.assertTrue(
				equals(
					OrderSerDes.toDTO(apiResponse),
					OrderSerDes.toDTO(vulcanResponse)));
		}
	}
