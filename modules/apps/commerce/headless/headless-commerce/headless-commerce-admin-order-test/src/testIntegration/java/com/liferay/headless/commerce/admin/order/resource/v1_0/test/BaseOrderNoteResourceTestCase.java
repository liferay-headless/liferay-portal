...
public abstract class BaseOrderNoteResourceTestCase {
	...
	@Test
	public void testGetOrderNoteMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		OrderNote postOrderNote = testGetOrderNote_addOrderNote();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-order/v1.0/orderNotes/{id}".replace(
				"{orderNoteId}", String.valueOf(postOrderNote.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/orderNotes/{id}".replace(
								"{orderNoteId}",
								String.valueOf(postOrderNote.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					OrderNoteSerDes.toDTO(apiResponse),
					OrderNoteSerDes.toDTO(vulcanResponse)));
		}
	}
