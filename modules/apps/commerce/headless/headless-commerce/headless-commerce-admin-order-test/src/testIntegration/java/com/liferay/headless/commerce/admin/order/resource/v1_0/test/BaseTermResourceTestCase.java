...
public abstract class BaseTermResourceTestCase {
	...
	@Test
	public void testGetTermMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Term postTerm = testGetTerm_addTerm();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-order/v1.0/terms/{id}".replace(
				"{termId}", String.valueOf(postTerm.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
			...
			Assert.assertTrue(
				equals(
					TermSerDes.toDTO(apiResponse),
					TermSerDes.toDTO(vulcanResponse)));
		}
	}
