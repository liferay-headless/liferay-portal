...
public abstract class BaseOrderRuleResourceTestCase {
	...
	@Test
	public void testGetOrderRuleMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		OrderRule postOrderRule = testGetOrderRule_addOrderRule();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-order/v1.0/order-rules/{id}".replace(
				"{orderRuleId}", String.valueOf(postOrderRule.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/order-rules/{id}".replace(
								"{orderRuleId}",
								String.valueOf(postOrderRule.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					OrderRuleSerDes.toDTO(apiResponse),
					OrderRuleSerDes.toDTO(vulcanResponse)));
		}
	}
