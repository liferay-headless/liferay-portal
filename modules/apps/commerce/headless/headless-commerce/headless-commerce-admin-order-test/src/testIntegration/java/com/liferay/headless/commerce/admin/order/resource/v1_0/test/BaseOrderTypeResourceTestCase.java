...
public abstract class BaseOrderTypeResourceTestCase {
	...
	@Test
	public void testGetOrderTypeMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		OrderType postOrderType = testGetOrderType_addOrderType();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-order/v1.0/order-types/{id}".replace(
				"{orderTypeId}", String.valueOf(postOrderType.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/order-types/{id}".replace(
								"{orderTypeId}",
								String.valueOf(postOrderType.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					OrderTypeSerDes.toDTO(apiResponse),
					OrderTypeSerDes.toDTO(vulcanResponse)));
		}
	}
