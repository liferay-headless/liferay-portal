...
public abstract class BaseWishListItemResourceTestCase {
	...
	@Test
	public void testGetWishListItemMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		WishListItem postWishListItem = testGetWishListItem_addWishListItem();

		String endpoint = StringBundler.concat(
			"/headless-commerce-delivery-catalog/v1.0/wishlist-items/{wishListItemId}".
				replace(
					"{wishListItemId}",
					String.valueOf(postWishListItem.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/wishlist-items/{wishListItemId}".
								replace(
									"{wishListItemId}",
									String.valueOf(postWishListItem.getId()));
						}
			...
			String vulcanResponse = vulcanCRUDItemDelegate.getItem(
				postWishListItem.getId()
			).toString();

			Assert.assertTrue(
				equals(
					WishListItemSerDes.toDTO(apiResponse),
					WishListItemSerDes.toDTO(vulcanResponse)));
		}
	}
