...
public abstract class BaseWishListResourceTestCase {
	...
	@Test
	public void testGetWishListMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		WishList postWishList = testGetWishList_addWishList();

		String endpoint = StringBundler.concat(
			"/headless-commerce-delivery-catalog/v1.0/wishlists/{wishListId}".
				replace("{wishListId}", String.valueOf(postWishList.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/wishlists/{wishListId}".replace(
								"{wishListId}",
								String.valueOf(postWishList.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					WishListSerDes.toDTO(apiResponse),
					WishListSerDes.toDTO(vulcanResponse)));
		}
	}
