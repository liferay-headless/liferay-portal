...
public abstract class BaseCartResourceTestCase {
	...
	@Test
	public void testGetCartMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Cart postCart = testGetCart_addCart();

		String endpoint = StringBundler.concat(
			"/headless-commerce-delivery-cart/v1.0/carts/{cartId}".replace(
				"{cartId}", String.valueOf(postCart.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/carts/{cartId}".replace(
								"{cartId}", String.valueOf(postCart.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					CartSerDes.toDTO(apiResponse),
					CartSerDes.toDTO(vulcanResponse)));
		}
	}
