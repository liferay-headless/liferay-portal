...
public abstract class BaseCartCommentResourceTestCase {
	...
	@Test
	public void testGetCartCommentMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		CartComment postCartComment = testGetCartComment_addCartComment();

		String endpoint = StringBundler.concat(
			"/headless-commerce-delivery-cart/v1.0/cart-comments/{cartCommentId}".
				replace(
					"{cartCommentId}",
					String.valueOf(postCartComment.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/cart-comments/{cartCommentId}".replace(
								"{cartCommentId}",
								String.valueOf(postCartComment.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					CartCommentSerDes.toDTO(apiResponse),
					CartCommentSerDes.toDTO(vulcanResponse)));
		}
	}
