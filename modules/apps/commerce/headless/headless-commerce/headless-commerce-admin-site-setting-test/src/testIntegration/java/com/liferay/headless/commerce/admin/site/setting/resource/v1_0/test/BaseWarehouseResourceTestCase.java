...
public abstract class BaseWarehouseResourceTestCase {
	...
	@Test
	public void testGetWarehouseMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Warehouse postWarehouse = testGetWarehouse_addWarehouse();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-site-setting/v1.0/warehouse/{id}".replace(
				"{warehouseId}", String.valueOf(postWarehouse.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/warehouse/{id}".replace(
								"{warehouseId}",
								String.valueOf(postWarehouse.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					WarehouseSerDes.toDTO(apiResponse),
					WarehouseSerDes.toDTO(vulcanResponse)));
		}
	}
