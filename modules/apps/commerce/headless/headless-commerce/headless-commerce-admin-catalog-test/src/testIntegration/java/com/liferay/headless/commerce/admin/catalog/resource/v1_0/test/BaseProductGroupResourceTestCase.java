...
public abstract class BaseProductGroupResourceTestCase {
	...
	@Test
	public void testGetProductGroupMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		ProductGroup postProductGroup = testGetProductGroup_addProductGroup();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-catalog/v1.0/product-groups/{id}".replace(
				"{productGroupId}", String.valueOf(postProductGroup.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/product-groups/{id}".replace(
								"{productGroupId}",
								String.valueOf(postProductGroup.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					ProductGroupSerDes.toDTO(apiResponse),
					ProductGroupSerDes.toDTO(vulcanResponse)));
		}
	}
