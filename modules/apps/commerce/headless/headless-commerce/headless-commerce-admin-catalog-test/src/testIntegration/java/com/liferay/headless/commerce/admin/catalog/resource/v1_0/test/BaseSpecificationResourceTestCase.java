...
public abstract class BaseSpecificationResourceTestCase {
	...
	@Test
	public void testGetSpecificationMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {
		...
		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-catalog/v1.0/specifications/{id}".replace(
				"{specificationId}",
				String.valueOf(postSpecification.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/specifications/{id}".replace(
								"{specificationId}",
								String.valueOf(postSpecification.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					SpecificationSerDes.toDTO(apiResponse),
					SpecificationSerDes.toDTO(vulcanResponse)));
		}
	}
