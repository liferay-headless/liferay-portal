...
public abstract class BaseCatalogResourceTestCase {
	...
	@Test
	public void testGetCatalogMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Catalog postCatalog = testGetCatalog_addCatalog();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-catalog/v1.0/catalog/{id}".replace(
				"{catalogId}", String.valueOf(postCatalog.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/catalog/{id}".replace(
								"{catalogId}",
								String.valueOf(postCatalog.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					CatalogSerDes.toDTO(apiResponse),
					CatalogSerDes.toDTO(vulcanResponse)));
		}
	}
