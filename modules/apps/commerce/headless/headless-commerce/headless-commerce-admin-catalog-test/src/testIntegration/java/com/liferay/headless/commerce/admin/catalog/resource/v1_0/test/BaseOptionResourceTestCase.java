...
public abstract class BaseOptionResourceTestCase {
	...
	@Test
	public void testGetOptionMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Option postOption = testGetOption_addOption();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-catalog/v1.0/options/{id}".replace(
				"{optionId}", String.valueOf(postOption.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/options/{id}".replace(
								"{optionId}",
								String.valueOf(postOption.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					OptionSerDes.toDTO(apiResponse),
					OptionSerDes.toDTO(vulcanResponse)));
		}
	}
