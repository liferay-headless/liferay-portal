...
public abstract class BaseRelatedProductResourceTestCase {
	...
	@Test
	public void testGetRelatedProductMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {
		...
		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-catalog/v1.0/relatedProducts/{id}".
				replace(
					"{relatedProductId}",
					String.valueOf(postRelatedProduct.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/relatedProducts/{id}".replace(
								"{relatedProductId}",
								String.valueOf(postRelatedProduct.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					RelatedProductSerDes.toDTO(apiResponse),
					RelatedProductSerDes.toDTO(vulcanResponse)));
		}
	}
