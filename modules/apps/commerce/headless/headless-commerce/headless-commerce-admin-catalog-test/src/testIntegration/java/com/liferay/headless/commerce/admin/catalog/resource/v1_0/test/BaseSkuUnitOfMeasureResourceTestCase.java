...
public abstract class BaseSkuUnitOfMeasureResourceTestCase {
	...
	@Test
	public void testGetSkuUnitOfMeasureMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {
		...
		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-catalog/v1.0/sku-unit-of-measures/{id}".
				replace(
					"{skuUnitOfMeasureId}",
					String.valueOf(postSkuUnitOfMeasure.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/sku-unit-of-measures/{id}".replace(
								"{skuUnitOfMeasureId}",
								String.valueOf(postSkuUnitOfMeasure.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					SkuUnitOfMeasureSerDes.toDTO(apiResponse),
					SkuUnitOfMeasureSerDes.toDTO(vulcanResponse)));
		}
	}
