...
public abstract class BaseCurrencyResourceTestCase {
	...
	@Test
	public void testGetCurrencyMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Currency postCurrency = testGetCurrency_addCurrency();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-catalog/v1.0/currencies/{id}".replace(
				"{currencyId}", String.valueOf(postCurrency.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/currencies/{id}".replace(
								"{currencyId}",
								String.valueOf(postCurrency.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					CurrencySerDes.toDTO(apiResponse),
					CurrencySerDes.toDTO(vulcanResponse)));
		}
	}
