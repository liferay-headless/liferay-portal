...
public abstract class BaseProductChannelResourceTestCase {
	...
	@Test
	public void testGetProductChannelMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {
		...
		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-catalog/v1.0/product-channels/{id}".
				replace(
					"{productChannelId}",
					String.valueOf(postProductChannel.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/product-channels/{id}".replace(
								"{productChannelId}",
								String.valueOf(postProductChannel.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					ProductChannelSerDes.toDTO(apiResponse),
					ProductChannelSerDes.toDTO(vulcanResponse)));
		}
	}
