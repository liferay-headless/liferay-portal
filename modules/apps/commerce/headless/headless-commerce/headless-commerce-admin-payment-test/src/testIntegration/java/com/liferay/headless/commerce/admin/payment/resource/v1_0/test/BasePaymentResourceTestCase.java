...
public abstract class BasePaymentResourceTestCase {
	...
	@Test
	public void testGetPaymentMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Payment postPayment = testGetPayment_addPayment();

		String endpoint = StringBundler.concat(
			"/headless-commerce-admin-payment/v1.0/payments/{id}".replace(
				"{paymentId}", String.valueOf(postPayment.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/payments/{id}".replace(
								"{paymentId}",
								String.valueOf(postPayment.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					PaymentSerDes.toDTO(apiResponse),
					PaymentSerDes.toDTO(vulcanResponse)));
		}
	}
