...
public class ToolSearchResultResourceTest
	...
	public void testGetToolSearchPage() throws Exception {
		...
		_testGetToolSearchPageWithAnActionEndpoint();
		_testGetToolSearchPageWithAnAmbiguousAssociation();
		_testGetToolSearchPageWithAnAssociation();
		_testGetToolSearchPageWithANestedScope();
		_testGetToolSearchPageWithAHeadNounBoundary();
		...
		_testGetToolSearchPageWithRequiredInputSchema();
	...
	private JSONObject _getInputSchemaJSONObject(JSONObject itemJSONObject)
		throws Exception {
		...
		JSONObject jsonObject = HTTPTestUtil.invokeToJSONObject(
			...
			StringBundler.concat(
				...
				itemJSONObject.getString("toolSetName"), "/tools/",
				itemJSONObject.getString("name")),
			Http.Method.GET);

		return jsonObject.getJSONObject("inputSchema");
	}
	...
	private void _testGetToolSearchPageWithAHeadNounBoundary()
		throws Exception {
	...
	private void _testGetToolSearchPageWithAnActionEndpoint() throws Exception {
		Assert.assertEquals(
			"postCTCollectionPublish",
			_getName(_searchToJSONArray("publish a change list"), 0));
	}

	private void _testGetToolSearchPageWithAnAmbiguousAssociation()
		throws Exception {
		...
		Assert.assertTrue(
			"An ambiguous association word paired with its direction word " +
				"did not reach an account tool: " + _getNames(jsonArray),
			StringUtil.startsWith(
				_getName(jsonArray, 0), "deleteAccountUserAccount"));
	}
	...
	private void _testGetToolSearchPageWithATurnedOffOperation()
		...
		JSONObject jsonObject = HTTPTestUtil.invokeToJSONObject(
			null, "oauth-client/v1.0/oauth-client-entries", Http.Method.GET);
	...
	private void _testGetToolSearchPageWithRequiredInputSchema()
		throws Exception {

		JSONArray jsonArray = _searchToJSONArray("create a blog entry");

		JSONObject itemJSONObject = jsonArray.getJSONObject(0);

		Assert.assertFalse(
			"A search that did not ask for the required arguments carried " +
				"them anyway",
			itemJSONObject.has("requiredInputSchema"));

		jsonArray = _search(
			true, 3, "create a blog entry"
		).getJSONArray(
			"items"
		);
		...
		JSONObject requiredInputSchemaJSONObject = itemJSONObject.getJSONObject(
			"requiredInputSchema");

		Assert.assertNotNull(
			_getNames(jsonArray), requiredInputSchemaJSONObject);

		JSONArray requiredJSONArray =
			requiredInputSchemaJSONObject.getJSONArray("required");

		Assert.assertTrue(
			requiredInputSchemaJSONObject.toString(),
			requiredJSONArray.length() > 0);

		JSONObject propertiesJSONObject =
			requiredInputSchemaJSONObject.getJSONObject("properties");

		for (int i = 0; i < requiredJSONArray.length(); i++) {
			String name = requiredJSONArray.getString(i);

			Assert.assertTrue(name, propertiesJSONObject.has(name));
		}

		Assert.assertEquals(
			requiredInputSchemaJSONObject.toString(),
			requiredJSONArray.length(),
			propertiesJSONObject.keySet(
			).size());

		JSONObject inputSchemaJSONObject = _getInputSchemaJSONObject(
			itemJSONObject);

		JSONObject fullPropertiesJSONObject =
			inputSchemaJSONObject.getJSONObject("properties");

		for (String name : propertiesJSONObject.keySet()) {
			Assert.assertTrue(
				name + " is missing from " + inputSchemaJSONObject,
				fullPropertiesJSONObject.has(name));
		}
	}
	...
	private void _testGetToolSearchPageWithRequiredInputSchemaOnAListing()
		...
		JSONArray jsonArray = _search(
			true, 1, "list the blog entries of a site"
		).getJSONArray(
			"items"
		);
		...
		Assert.assertTrue(
			_getName(jsonArray, 0),
			StringUtil.endsWith(_getName(jsonArray, 0), "Page"));
		...
		Assert.assertTrue(
			requiredInputSchemaJSONObject.toString(),
			propertiesJSONObject.has("fields"));
	}
	...
	private void _testGetToolSearchPageWithTheDefaultProfile()
		...
		Assert.assertFalse(
			"The default profile offers the search tool: " + toolNames,
			toolNames.contains("getToolSearchPage"));
	}
	...
	private void _waitForToolIndex() throws Exception {
		IdempotentRetryAssert.retryAssert(
			60, TimeUnit.SECONDS, 500, TimeUnit.MILLISECONDS,
			() -> {
				JSONObject jsonObject = HTTPTestUtil.invokeToJSONObject(
					null,
					"mcp-server/v1.0/tool-search?search=" +
						URLCodec.encodeURL("create a blog entry"),
					Http.Method.GET);

				Assert.assertTrue(
					"The tool index is not populated: " + jsonObject,
					jsonObject.getInt("totalCount") > 0);
