...
public class SearchToolUtil {
	...
	public static Page<ToolSearchResult> getSearchToolsPage(
		...
		boolean includeRequiredInputSchema, int limit,
		MCPToolIndexReader mcpToolIndexReader,
		MCPToolIndexWriter mcpToolIndexWriter, String search) {

		if (!isToolSearchSupported()) {
			throw new ToolSearchUnavailableException(
				MCPClientHints.TOOL_SEARCH_UNSUPPORTED);
		}
		...
		try {
			return Page.of(
				_getToolSearchResults(
					...
					limit, mcpToolIndexReader, search));
		}
		catch (RuntimeException runtimeException) {
			_log.error("Unable to search the tool index", runtimeException);

			throw new ToolSearchUnavailableException(
				MCPClientHints.TOOL_SEARCH_UNAVAILABLE);
		}
	}

	public static boolean isToolSearchSupported() {
	...
	private static Map<String, Object> _getRequiredInputSchema(
		...
		try {
			...
			Map<String, Object> requiredInputSchema =
				ToolSetUtil.toRequiredInputSchemaOnly(inputSchema);
	...
	private static List<ToolSearchResult> _getToolSearchResults(
		...
		boolean includeRequiredInputSchema, int limit,
		MCPToolIndexReader mcpToolIndexReader, String search) {

		List<ToolSearchResult> toolSearchResults = mcpToolIndexReader.search(
			companyId, _getConfidenceMargin(), includeRequiredInputSchema,
			_getLimit(companyId, limit), search);
