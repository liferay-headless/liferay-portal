...
public class OpenAPIBriefUtil {
	...
	public static void clearOpenAPIJSONObjectCache(
		...
		String toolSetName = getToolSetName(companyId, restContextPath);
	...
	public static OpenAPIBrief getOpenAPIBrief(
		long companyId, String toolSetName) {

		Map<String, OpenAPIBrief> openAPIBriefs = getOpenAPIBriefs(companyId);
	...
	public static Map<String, OpenAPIBrief> getOpenAPIBriefs(long companyId) {
		...
		if (changeCount == -1) {
			return _buildOpenAPIBriefs(companyId);
		}

		return _openAPIBriefs.computeIfAbsent(
			companyId, key -> _buildOpenAPIBriefs(companyId));
	}
	...
	public static String getToolSetName(
		long companyId, String restContextPath) {
		...
		Map<String, OpenAPIBrief> openAPIBriefs = _getCachedOpenAPIBriefs(
			companyId);

		String basePath = null;
		String toolSetName = null;

		for (Map.Entry<String, OpenAPIBrief> entry : openAPIBriefs.entrySet()) {
		...
		if (!restContextPath.startsWith(_OBJECT_CONTEXT_PATH_PREFIX)) {
			return null;
		}
	...
	private static Map<String, OpenAPIBrief> _buildOpenAPIBriefs(
		...
		for (ApplicationDTO applicationDTO : runtimeDTO.applicationDTOs) {
			...
			if (Validator.isNull(base) ||
				!_isAvailable(applicationDTO, bundleContext, companyId)) {
	...
	private static Map<String, OpenAPIBrief> _getCachedOpenAPIBriefs(
		...
		Map<String, OpenAPIBrief> openAPIBriefs = _openAPIBriefs.get(companyId);
	...
	private static boolean _isAvailable(
		ApplicationDTO applicationDTO, BundleContext bundleContext,
		long companyId) {
		...
		try {
			serviceReferences = bundleContext.getAllServiceReferences(
				null, "(service.id=" + applicationDTO.serviceId + ")");
		}
		catch (InvalidSyntaxException invalidSyntaxException) {
			throw new IllegalStateException(invalidSyntaxException);
		}
		...
		List<String> companyIds = StringPlus.asList(
			serviceReferences[0].getProperty("companyId"));

		if (companyIds.isEmpty()) {
			return true;
		}

		return companyIds.contains(String.valueOf(companyId));
	}

	private static final String _OBJECT_CONTEXT_PATH_PREFIX = "/c/";
