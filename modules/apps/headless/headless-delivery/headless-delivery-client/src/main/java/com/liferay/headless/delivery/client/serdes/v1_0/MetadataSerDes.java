...
import com.liferay.headless.delivery.client.dto.v1_0.Metadata;
...
public class MetadataSerDes {
	...
	public static String toJSON(Metadata metadata) {
		...
		if (metadata.getContentFields() != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			for (int i = 0; i < metadata.getContentFields().length; i++) {
				sb.append(String.valueOf(metadata.getContentFields()[i]));

				if ((i + 1) < metadata.getContentFields().length) {
					sb.append(", ");
				}
			}
		...
		if (metadata.getName() != null) {
			...
			sb.append(_escape(metadata.getName()));

			sb.append("\"");
		}

		if (metadata.getName_i18n() != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			sb.append(_toJSON(metadata.getName_i18n()));
		}
	...
	public static Map<String, String> toMap(Metadata metadata) {
		...
		if (metadata.getContentFields() == null) {
			...
			map.put(
				"contentFields", String.valueOf(metadata.getContentFields()));
		}
		...
		if (metadata.getName() == null) {
			...
			map.put("name", String.valueOf(metadata.getName()));
		}
		...
		if (metadata.getName_i18n() == null) {
			...
			map.put("name_i18n", String.valueOf(metadata.getName_i18n()));
		}
	...
	public static class MetadataJSONParser extends BaseJSONParser<Metadata> {
		...
		protected void setField(
			...
			if (Objects.equals(jsonParserFieldName, "contentFields")) {
				...
				if (jsonParserFieldValue != null) {
					metadata.setName((String)jsonParserFieldValue);
				}
			}
			else if (Objects.equals(jsonParserFieldName, "name_i18n")) {
				if (jsonParserFieldValue != null) {
					metadata.setName_i18n(
						(Map)MetadataSerDes.toMap(
