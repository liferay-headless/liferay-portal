...
public abstract class BaseWikiPageResourceTestCase {
	...
	@Test
	public void testGetWikiPageMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		WikiPage postWikiPage = testGetWikiPage_addWikiPage();

		String endpoint = StringBundler.concat(
			"/headless-delivery/v1.0/wiki-pages/{wikiPageId}".replace(
				"{wikiPageId}", String.valueOf(postWikiPage.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/wiki-pages/{wikiPageId}".replace(
								"{wikiPageId}",
								String.valueOf(postWikiPage.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					WikiPageSerDes.toDTO(apiResponse),
					WikiPageSerDes.toDTO(vulcanResponse)));
		}
	}
