...
public abstract class BaseWikiNodeResourceTestCase {
	...
	@Test
	public void testGetWikiNodeMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		WikiNode postWikiNode = testGetWikiNode_addWikiNode();

		String endpoint = StringBundler.concat(
			"/headless-delivery/v1.0/wiki-nodes/{wikiNodeId}".replace(
				"{wikiNodeId}", String.valueOf(postWikiNode.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/wiki-nodes/{wikiNodeId}".replace(
								"{wikiNodeId}",
								String.valueOf(postWikiNode.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					WikiNodeSerDes.toDTO(apiResponse),
					WikiNodeSerDes.toDTO(vulcanResponse)));
		}
	}
