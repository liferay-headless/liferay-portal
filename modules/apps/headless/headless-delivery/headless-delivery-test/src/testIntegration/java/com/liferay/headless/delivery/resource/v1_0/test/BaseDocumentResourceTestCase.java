...
public abstract class BaseDocumentResourceTestCase {
	...
	@Test
	public void testGetDocumentMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Document postDocument = testGetDocument_addDocument();

		String endpoint = StringBundler.concat(
			"/headless-delivery/v1.0/documents/{documentId}".replace(
				"{documentId}", String.valueOf(postDocument.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/documents/{documentId}".replace(
								"{documentId}",
								String.valueOf(postDocument.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					DocumentSerDes.toDTO(apiResponse),
					DocumentSerDes.toDTO(vulcanResponse)));
		}
	}
