...
public abstract class BaseCountryResourceTestCase {
	...
	@Test
	public void testGetCountryMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Country postCountry = testGetCountry_addCountry();

		String endpoint = StringBundler.concat(
			"/headless-admin-address/v1.0/countries/{countryId}".replace(
				"{countryId}", String.valueOf(postCountry.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/countries/{countryId}".replace(
								"{countryId}",
								String.valueOf(postCountry.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					CountrySerDes.toDTO(apiResponse),
					CountrySerDes.toDTO(vulcanResponse)));
		}
	}
