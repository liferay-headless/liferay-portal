...
public abstract class BaseRegionResourceTestCase {
	...
	@Test
	public void testGetRegionMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Region postRegion = testGetRegion_addRegion();

		String endpoint = StringBundler.concat(
			"/headless-admin-address/v1.0/regions/{regionId}".replace(
				"{regionId}", String.valueOf(postRegion.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/regions/{regionId}".replace(
								"{regionId}",
								String.valueOf(postRegion.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					RegionSerDes.toDTO(apiResponse),
					RegionSerDes.toDTO(vulcanResponse)));
		}
	}
