...
public abstract class BaseUserGroupResourceTestCase {
	...
	@Test
	public void testGetAssetLibraryUserGroupsPageWithSortCollection()
		throws Exception {

		JSONObject xSortableJSONObject = HTTPTestUtil.invokeToJSONObject(
			null, "headless-asset-library/v1.0/openapi.json", Http.Method.GET
		).getJSONObject(
			"components"
		).getJSONObject(
			"schemas"
		...
		for (EntityField entityField :
			...
			try {
				Method method = userGroup1.getClass(
				).getMethod(
					"get" + StringUtil.upperCaseFirstLetter(entityFieldName)
				);
				...
				if (returnType.equals(Long.class)) {
					BeanTestUtil.setProperty(userGroup1, entityFieldName, 0L);
					BeanTestUtil.setProperty(userGroup2, entityFieldName, 1L);
				}
				else if (returnType.equals(Integer.class)) {
					BeanTestUtil.setProperty(userGroup1, entityFieldName, 0);
					BeanTestUtil.setProperty(userGroup2, entityFieldName, 1);
				}
...
// LIFERAY-REST-BUILDER-HASH:1490598334