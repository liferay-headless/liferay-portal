...
public abstract class BaseWorkflowDefinitionResourceTestCase {
	...
	@Test
	public void testGetWorkflowDefinitionsPageWithSortCollection()
		throws Exception {

		JSONObject xSortableJSONObject = HTTPTestUtil.invokeToJSONObject(
			null, "headless-admin-workflow/v1.0/openapi.json", Http.Method.GET
		).getJSONObject(
			"components"
		).getJSONObject(
			"schemas"
		...
		for (EntityField entityField :
			...
			try {
				Method method = workflowDefinition1.getClass(
				).getMethod(
					"get" + StringUtil.upperCaseFirstLetter(entityFieldName)
				);
				...
				if (returnType.equals(Long.class)) {
					BeanTestUtil.setProperty(
						workflowDefinition1, entityFieldName, 0L);
					BeanTestUtil.setProperty(
						workflowDefinition2, entityFieldName, 1L);
				}
				else if (returnType.equals(Integer.class)) {
					BeanTestUtil.setProperty(
						workflowDefinition1, entityFieldName, 0);
					BeanTestUtil.setProperty(
						workflowDefinition2, entityFieldName, 1);
				}
...
// LIFERAY-REST-BUILDER-HASH:-129375287