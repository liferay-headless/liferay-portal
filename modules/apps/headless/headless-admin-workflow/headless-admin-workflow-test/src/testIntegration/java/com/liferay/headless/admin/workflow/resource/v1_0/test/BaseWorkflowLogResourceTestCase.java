...
public abstract class BaseWorkflowLogResourceTestCase {
	...
	@Test
	public void testGetWorkflowLogMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		WorkflowLog postWorkflowLog = testGetWorkflowLog_addWorkflowLog();

		String endpoint = StringBundler.concat(
			"/headless-admin-workflow/v1.0/workflow-logs/{workflowLogId}".
				replace(
					"{workflowLogId}",
					String.valueOf(postWorkflowLog.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/workflow-logs/{workflowLogId}".replace(
								"{workflowLogId}",
								String.valueOf(postWorkflowLog.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					WorkflowLogSerDes.toDTO(apiResponse),
					WorkflowLogSerDes.toDTO(vulcanResponse)));
		}
	}
