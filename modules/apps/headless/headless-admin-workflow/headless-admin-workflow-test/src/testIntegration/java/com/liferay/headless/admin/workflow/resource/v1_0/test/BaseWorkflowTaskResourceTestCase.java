...
public abstract class BaseWorkflowTaskResourceTestCase {
	...
	@Test
	public void testGetWorkflowTaskMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		WorkflowTask postWorkflowTask = testGetWorkflowTask_addWorkflowTask();

		String endpoint = StringBundler.concat(
			"/headless-admin-workflow/v1.0/workflow-tasks/{workflowTaskId}".
				replace(
					"{workflowTaskId}",
					String.valueOf(postWorkflowTask.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/workflow-tasks/{workflowTaskId}".
								replace(
									"{workflowTaskId}",
									String.valueOf(postWorkflowTask.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					WorkflowTaskSerDes.toDTO(apiResponse),
					WorkflowTaskSerDes.toDTO(vulcanResponse)));
		}
	}
