...
public class HeadlessBuilderResourceTest extends BaseTestCase {
	...
	public void testGetWithRelatedModel() throws Exception {
		...
		responseJSONObject = HTTPTestUtil.invokeToJSONObject(
			null,
			"c/" + apiApplicationJSONObject.getString("baseURL") +
				apiEndpointJSONObject.getString("path"),
			Http.Method.GET);

		itemsJSONArray = responseJSONObject.getJSONArray("items");

		jsonObject = itemsJSONArray.getJSONObject(1);

		apiEndpointsERCJSONArray = jsonObject.getJSONArray("APIEndpointsERC");

		Assert.assertEquals("[]", apiEndpointsERCJSONArray.toString());
	}
