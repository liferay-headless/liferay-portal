...
public abstract class BaseFormResourceTestCase {
	...
	@Test
	public void testGetFormMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Form postForm = testGetForm_addForm();

		String endpoint = StringBundler.concat(
			"/headless-form/v1.0/forms/{formId}".replace(
				"{formId}", String.valueOf(postForm.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/forms/{formId}".replace(
								"{formId}", String.valueOf(postForm.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					FormSerDes.toDTO(apiResponse),
					FormSerDes.toDTO(vulcanResponse)));
		}
	}
