...
public abstract class BaseFormRecordResourceTestCase {
	...
	@Test
	public void testGetFormRecordMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		FormRecord postFormRecord = testGetFormRecord_addFormRecord();

		String endpoint = StringBundler.concat(
			"/headless-form/v1.0/form-records/{formRecordId}".replace(
				"{formRecordId}", String.valueOf(postFormRecord.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/form-records/{formRecordId}".replace(
								"{formRecordId}",
								String.valueOf(postFormRecord.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					FormRecordSerDes.toDTO(apiResponse),
					FormRecordSerDes.toDTO(vulcanResponse)));
		}
	}
