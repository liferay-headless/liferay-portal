...
public abstract class BaseEmailAddressResourceTestCase {
	...
	@Test
	public void testGetEmailAddressMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		EmailAddress postEmailAddress = testGetEmailAddress_addEmailAddress();

		String endpoint = StringBundler.concat(
			"/headless-admin-user/v1.0/email-addresses/{emailAddressId}".
				replace(
					"{emailAddressId}",
					String.valueOf(postEmailAddress.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/email-addresses/{emailAddressId}".
								replace(
									"{emailAddressId}",
									String.valueOf(postEmailAddress.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					EmailAddressSerDes.toDTO(apiResponse),
					EmailAddressSerDes.toDTO(vulcanResponse)));
		}
	}
