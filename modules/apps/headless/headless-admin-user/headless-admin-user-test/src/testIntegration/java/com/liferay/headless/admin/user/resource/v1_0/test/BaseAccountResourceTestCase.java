...
public abstract class BaseAccountResourceTestCase {
	...
	@Test
	public void testGetAccountMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Account postAccount = testGetAccount_addAccount();

		String endpoint = StringBundler.concat(
			"/headless-admin-user/v1.0/accounts/{accountId}".replace(
				"{accountId}", String.valueOf(postAccount.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/accounts/{accountId}".replace(
								"{accountId}",
								String.valueOf(postAccount.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					AccountSerDes.toDTO(apiResponse),
					AccountSerDes.toDTO(vulcanResponse)));
		}
	}
