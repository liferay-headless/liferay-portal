...
public abstract class BaseWebUrlResourceTestCase {
	...
	@Test
	public void testGetWebUrlMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		WebUrl postWebUrl = testGetWebUrl_addWebUrl();

		String endpoint = StringBundler.concat(
			"/headless-admin-user/v1.0/web-urls/{webUrlId}".replace(
				"{webUrlId}", String.valueOf(postWebUrl.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/web-urls/{webUrlId}".replace(
								"{webUrlId}",
								String.valueOf(postWebUrl.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					WebUrlSerDes.toDTO(apiResponse),
					WebUrlSerDes.toDTO(vulcanResponse)));
		}
	}
