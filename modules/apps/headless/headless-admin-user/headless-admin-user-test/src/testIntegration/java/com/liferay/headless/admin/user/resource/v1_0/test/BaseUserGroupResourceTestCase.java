...
public abstract class BaseUserGroupResourceTestCase {
	...
	@Test
	public void testGetUserGroupMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		UserGroup postUserGroup = testGetUserGroup_addUserGroup();

		String endpoint = StringBundler.concat(
			"/headless-admin-user/v1.0/user-groups/{userGroupId}".replace(
				"{userGroupId}", String.valueOf(postUserGroup.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/user-groups/{userGroupId}".replace(
								"{userGroupId}",
								String.valueOf(postUserGroup.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					UserGroupSerDes.toDTO(apiResponse),
					UserGroupSerDes.toDTO(vulcanResponse)));
		}
	}
