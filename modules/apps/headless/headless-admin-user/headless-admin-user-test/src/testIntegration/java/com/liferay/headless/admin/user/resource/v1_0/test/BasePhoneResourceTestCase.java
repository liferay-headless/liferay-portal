...
public abstract class BasePhoneResourceTestCase {
	...
	@Test
	public void testGetPhoneMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Phone postPhone = testGetPhone_addPhone();

		String endpoint = StringBundler.concat(
			"/headless-admin-user/v1.0/phones/{phoneId}".replace(
				"{phoneId}", String.valueOf(postPhone.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/phones/{phoneId}".replace(
								"{phoneId}", String.valueOf(postPhone.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					PhoneSerDes.toDTO(apiResponse),
					PhoneSerDes.toDTO(vulcanResponse)));
		}
	}
