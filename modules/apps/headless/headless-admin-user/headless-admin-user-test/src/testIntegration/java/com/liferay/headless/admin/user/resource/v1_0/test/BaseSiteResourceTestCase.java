...
public abstract class BaseSiteResourceTestCase {
	...
	@Test
	public void testGetSiteMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Site postSite = testGetSite_addSite();

		String endpoint = StringBundler.concat(
			"/headless-admin-user/v1.0/sites/{siteId}".replace(
				"{siteId}", String.valueOf(postSite.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/sites/{siteId}".replace(
								"{siteId}", String.valueOf(postSite.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					SiteSerDes.toDTO(apiResponse),
					SiteSerDes.toDTO(vulcanResponse)));
		}
	}
