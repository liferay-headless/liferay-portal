...
public abstract class BaseRoleResourceTestCase {
	...
	@Test
	public void testGetRoleMatchesVulcanCRUDItemDelegateGetItem()
		throws Exception {

		Role postRole = testGetRole_addRole();

		String endpoint = StringBundler.concat(
			"/headless-admin-user/v1.0/roles/{roleId}".replace(
				"{roleId}", String.valueOf(postRole.getId())));
		...
		if (vulcanCRUDItemDelegateBuilder != null) {
				...
				vulcanCRUDItemDelegateBuilder.acceptLanguage(
					...
					new UriInfo() {
						...
						public String getPath() {
							return "v1.0/roles/{roleId}".replace(
								"{roleId}", String.valueOf(postRole.getId()));
						}
			...
			Assert.assertTrue(
				equals(
					RoleSerDes.toDTO(apiResponse),
					RoleSerDes.toDTO(vulcanResponse)));
		}
	}
