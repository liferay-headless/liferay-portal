...
public abstract class BaseStyleBookResourceTestCase {
	...
	@Test
	public void testGetDesignLibraryStyleBooksPageWithSortCollection()
		throws Exception {
		...
		for (EntityField entityField :
			...
			try {
				Method method = styleBook1.getClass(
				).getMethod(
					"get" + StringUtil.upperCaseFirstLetter(entityFieldName)
				);
				...
				if (returnType.equals(Long.class)) {
					BeanTestUtil.setProperty(styleBook1, entityFieldName, 0L);
					BeanTestUtil.setProperty(styleBook2, entityFieldName, 1L);
				}
				else if (returnType.equals(Integer.class)) {
					BeanTestUtil.setProperty(styleBook1, entityFieldName, 0);
					BeanTestUtil.setProperty(styleBook2, entityFieldName, 1);
				}
	...
	@Test
	public void testGetSiteStyleBooksPageWithSortCollection() throws Exception {
		JSONObject xSortableJSONObject = HTTPTestUtil.invokeToJSONObject(
			null, "headless-admin-site/v1.0/openapi.json", Http.Method.GET
		).getJSONObject(
			"components"
...
// LIFERAY-REST-BUILDER-HASH:-2145459757