...
public class SamlProviderResourceImpl extends BaseSamlProviderResourceImpl {
	...
	public SamlProvider getSamlProvider() throws Exception {
		...
		SamlProvider.Role samlProviderRole = samlProvider.getRole();
		...
		samlProvider.setRole(
			() -> {
				...
				else if (SamlProviderConfigurationKeys.SAML_ROLE_IDP.equals(
							role)) {

					return SamlProvider.Role.IDP;
				}

				return samlProviderRole;
			});
		...
		samlProvider.setIdp(
			() -> {
				if (SamlProviderConfigurationKeys.SAML_ROLE_IDP.equals(role)) {
