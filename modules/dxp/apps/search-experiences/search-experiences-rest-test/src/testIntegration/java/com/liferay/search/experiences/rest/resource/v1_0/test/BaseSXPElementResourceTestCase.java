...
public abstract class BaseSXPElementResourceTestCase {
	...
	@Test
	public void testGetSXPElementsPageWithSortCollection() throws Exception {
		JSONObject xSortableJSONObject = HTTPTestUtil.invokeToJSONObject(
			null, "search-experiences-rest/v1.0/openapi.json", Http.Method.GET
		).getJSONObject(
			"components"
		).getJSONObject(
			"schemas"
		...
		for (EntityField entityField :
			...
			try {
				Method method = sxpElement1.getClass(
				).getMethod(
					"get" + StringUtil.upperCaseFirstLetter(entityFieldName)
				);
				...
				if (returnType.equals(Long.class)) {
					BeanTestUtil.setProperty(sxpElement1, entityFieldName, 0L);
					BeanTestUtil.setProperty(sxpElement2, entityFieldName, 1L);
				}
				else if (returnType.equals(Integer.class)) {
					BeanTestUtil.setProperty(sxpElement1, entityFieldName, 0);
					BeanTestUtil.setProperty(sxpElement2, entityFieldName, 1);
				}
...
// LIFERAY-REST-BUILDER-HASH:383646977